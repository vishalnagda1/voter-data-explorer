...
Roll type, municipality or panchayat name, ward, and part metadata are detected from the PDF header using OCR cross-checked against the embedded text layer. PDF filenames are not required to follow a convention; a matching conventional filename may only enrich a location label after the document content confirms it. Explicit command-line metadata options remain available as authoritative overrides.
